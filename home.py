        ...
        if check_sharetoken(token_result):
            ...
            if web_setting["web"]["refresh_all"]:
                if str(config[user_name]["expires_in"]) == "0":
                    ...
                    logging.info(f"<用户> 【SA验证】 {user_name} 的SA_Token已经失效！尝试刷新中...")
                    ...
                    error_status = "过期"
                    logging.error(f"<用户> 【SA验证】 {user_name} 的SA_Token已经过期！")
            else:
                error_status = "SA已过期"
                logging.error(f"<用户> 【SA验证】 {user_name} 的SA_Token已经过期！")
...
def select():
    ...
    if st.button("**加入我们**", use_container_width=True, type=button_type):
        if user_new_acc == "":
            sac.alert(label="**请填写您的账户！**", color="error", variant="filled", size="sm", radius="sm", icon=True)
        elif user_new_pass == "":
            sac.alert(label="**请填写您的密码！**", color="error", variant="filled", size="sm", radius="sm", icon=True)
        elif len(user_new_pass) < 8:
            sac.alert(label="**密码长度至少为8位！**", color="error", variant="filled", size="sm", radius="sm", icon=True)
        elif user_new_acc in config:
            sac.alert(label="**此账户已存在！**", color="error", variant="filled", size="sm", radius="sm", icon=True)
        elif user_new_invite not in invite_config.keys():
            sac.alert(label="**邀请令牌不存在！**", color="error", variant="filled", size="sm", radius="sm", icon=True)
    ...
    with col6:
        if web_setting["web"]["notice_enable"]:
            sac.alert(label=web_setting["web"]["notice_title"], description=web_setting["web"]["notice_subtitle"],
                      ...
                      size=web_setting["web"]["notice_size"], radius=web_setting["web"]["notice_radius"],
                      key="free_notice1")
    with col5:
    ...
    with col5:
        if web_setting["web"]["notice_enable"]:
            sac.alert(label=web_setting["web"]["notice_title"],
                      ...
                      size=web_setting["web"]["notice_size"], radius=web_setting["web"]["notice_radius"], key="free_notice")
        st.write("")
