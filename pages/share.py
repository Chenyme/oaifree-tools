    ...
    if web_setting["web"]["choose_domain"] == "不允许":
        ...
        with st.status("**正在验证您的身份...**") as status:
            if error_status == "RF过期":
                ...
                st.write("")
                sac.alert(label="**验证失败，请联系管理员！**", color="error", variant="filled", size="md", radius="md",
                          icon=True, closable=True)
            elif error_status == "过期":
                status.update(label="**啊哦！出错了！**", state="error", expanded=True)
                st.write("")
                sac.alert(label="**账户已过期，请联系管理员续费！**", color="error", variant="filled", size="md",
                          radius="md", icon=True, closable=True)
            else:
