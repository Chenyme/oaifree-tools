...
if "click" in st.session_state:
    if "refresh_click" in st.session_state:
        del st.session_state["refresh_click"]
    if "invite_click" in st.session_state:
        del st.session_state["invite_click"]
    if "add_account" in st.session_state:
        del st.session_state["add_account"]
    if "refresh_user_click" in st.session_state:
        del st.session_state["refresh_user_click"]
    if "add_domain_click" in st.session_state:
        del st.session_state["add_domain_click"]
...
if st.session_state.role == "admin":
    ...
    def download():
        with st.expander("**数据文件说明**", expanded=False, icon=":material/description:"):
            st.info("""
            **app.log： 运行日志**
            ...
            **请下载后妥善保管以上文件，且勿随意泄露，以免造成不必要的损失！**
            """)
        my_bar = st.progress(0, text="**Loading...**")

        files = ["app.log", "users.json", "share.json", "domain.json", "setting.toml", "accounts.json", "invite.json", "refresh.json", "openai.json", "anthropic.json", "secret.toml"]
        ...
        try:
            ...
            if st.download_button(label="**下载数据**", data=zip_buffer, file_name="config.zip", mime="application/zip", use_container_width=True, type=button_type):
                st.balloons()
        except:
            st.write("")
    ...
    def upload():
        ...
        **请慎重上传 V1.1.x 或 更早版本 的配置文件！**
        ...
        required_files = ["app.log", "users.json", "share.json", "domain.json", "setting.toml", "accounts.json", "invite.json", "refresh.json", "openai.json", "anthropic.json", "secret.toml"]
            ...
            if uploaded_file is not None:
                ...
                with zipfile.ZipFile(zip_buffer, "r") as zip_ref:
                    ...
                    for file in error_files:
                        error = True
                        st.error(f"**已阻止未知文件{file}**！", icon=":material/error:")
                    if not error:
                        st.balloons()
                        with st.status("Downloading data...") as status:
                            status.update(label="**即 将 自 动 刷 新 ... 3**", state="complete", expanded=False)
                            time.sleep(1)
                            status.update(label="**即 将 自 动 刷 新 ... 2**", state="complete", expanded=False)
                            time.sleep(1)
    ...
    def add_domain():
        ...
        if st.button("**新增此服务**", use_container_width=True, type=button_type):
            if new_domain_name == "":
                st.write("")
                st.error("**服务名称不可为空，请重新填写！**", icon=":material/error:")
            elif new_domain_url == "":
                st.write("")
                st.error("**服务链接不可为空，请重新填写！**", icon=":material/error:")
            elif new_domain_url in domain_data.keys():
                st.write("")
                st.error("**此服务链接已存在，请重新填写！**", icon=":material/error:")
            else:
    ...
    def new_account():
        ...
        if new_service_provider == "Anthropic":
    ...
    def refresh_user():
        ...
        if st.button("**状态检测**", use_container_width=True, key="check_user_account", type=button_type):
            if len(status_list) != 0:
                ...
                if len(er_list) == 0:
                    ...
                    st.info("**滚动表格查看完整名单**", icon=":material/double_arrow:")
        ...
        st.markdown("""<style>.divider {margin-top: -20px;}</style><div class="divider"><hr></div>""", unsafe_allow_html=True)
        st.write("**需要刷新的账户**")
    ...
    with st.sidebar:
        ...
        <div style="text-align: center; margin-top: 20px;">
            <div class="custom-element" style="animation: subtleMove 10s ease-in-out infinite;">欢迎</div>
            <br>
            <div class="custom-element" style="animation: subtleMove 10s ease-in-out infinite;">""" + web_setting["web"]["super_user"] + """</div>
        </div>
        """
        ...
        set_choose = sac.menu([
            ...
            sac.MenuItem('号池管理', icon='box-seam-fill', children=[
                sac.MenuItem('账户总览', icon='diagram-3'),
            ...
            sac.MenuItem('更多功能', icon='basket2-fill', children=[
                ...
                sac.MenuItem('Linux.Do', icon='database'),
                sac.MenuItem('关于项目', icon='info-circle')]),
        ], open_all=False)

        st.markdown("""<style>.divider {margin-top: -25px;}</style><div class="divider"><hr></div>""", unsafe_allow_html=True)
        if st.button("**返回首页**"):
    ...
    if set_choose == '服务总览':
        ...
        with st.container(border=True):
            ...
            col5.metric("**配置占用**", size2, size_bytes2 - 13005)
            ...
            **由于最新版本 V1.2.x 的配置文件进行了重构，请慎重上传 V1.1.x 或 更早版本 的配置文件！**
            ...
            with col1:
                ...
                if st.button("**清理日志**", use_container_width=True, type=button_type):
                    result_str = "【OaiT服务已成功开启！】\n"
                    with open(current_path + "/app.log", "w", encoding="utf-8") as log_file:
                        log_file.write(result_str)
                    st.toast("**日志已清空！**", icon=":material/check_circle:")
            log_area.text_area("运行日志", value=result_str, height=300, label_visibility="collapsed")
            st.write("")
    if set_choose == '域名管理':
        ...
        with st.container(border=True):
            st.write("")
            st.write("##### 域名列表")
            st.info("**Fuclaude 指向 claude UI， Oiafree 指向 ChatGPT UI， Pandora 指向 Pandora 经典UI**", icon=':material/double_arrow:')
            st.write("")

            @st.experimental_fragment
            def domain_area():
                rows = [[datas['name'], name, datas['speed'], datas['type']] for name, datas in domain_data.items()]
                st.session_state.df0 = pd.DataFrame(rows, columns=["名称", "服务域名", "延迟", "类型"])
                data = st.data_editor(st.session_state.df0, hide_index=True, use_container_width=True, height=248, num_rows="dynamic", disabled=["延迟"], column_config={"类型": st.column_config.SelectboxColumn(options=["Fuclaude", "Oaifree", "Pandora"])})
                ...
                with col6:
                    if st.button("**保存更改**", use_container_width=True, key="save_domain", type=button_type):
                        ...
                        domain_data_json = df_to_json_domain(data)
                        with open(current_path + "domain.json", "w", encoding="utf-8") as json_file:
                            json_file.write(domain_data_json)
                        st.session_state.success = True
                        st.rerun()
                with col7:
                ...
                with col8:
                    if st.button("**新增域名**", use_container_width=True, type=button_type):
                        st.session_state.add_domain_click = True
                        time.sleep(0.05)
                        st.rerun()
            domain_area()
            ...
            @st.experimental_fragment
            def domain_setting_area():
                openai_domain = ["不启用"]
                anthropic_domain = ["不启用"]
                for domain in st.session_state.df0["服务域名"].tolist():
                ...
                if choose_domain == "不允许":
                    ...
                    try:
                        user_domain = st.multiselect("**用户可使用的服务域名**", st.session_state.df0["服务域名"].tolist(), default=web_setting["domain"]["domain_select"])
                    except:
                        if "change_domain" in st.session_state:
                            st.write("")
                        ...
                        user_domain = st.multiselect("**用户可使用的服务域名**", st.session_state.df0["服务域名"].tolist(), key="user_domain")
                    ...
                    if st.button("**保存更改**", use_container_width=True, key="save_domain_setting2", type=button_type):
                        ...
                        if choose_domain == "不允许":
                            ...
                            if len(user_domain) == 0:
                                with col3:
                                    st.error("**允许用户使用的域名不能为空！**", icon=':material/error:')
    ...
    if set_choose == '主题样式':
        ...
        with st.container(border=True):
            ...
            st.divider()
            st.write("##### Retro Orange 页面设计")
            st.info("**Retro Orange 主题可自由设计登录面板下方的所有内容，支持 Html 和 Markdown 语法！**", icon=':material/double_arrow:')
            st.write("")

            @st.experimental_fragment
            def theme_Retro_design_area():
                html = st.text_area("**页面设计**", value=web_setting['web']['Retro_Orange_notice'], height=400)
                col6, col7 = st.columns([0.2, 0.8])
                with col6:
                    if st.button("**保存更改**", use_container_width=True, key="save_design", type=button_type):
                        ...
                        st.rerun()
                with col7:
                    preview = st.toggle("**预览**", False, key="theme_Retro_design_area")
                if preview:
                    st.write("")
                    st.write("")
    ...
    if set_choose == '账户总览':
        with st.container(border=True):
            ...
            **注：Anthropic 无 RF_Token ，只需在 AC_Token 内填入 Claude 的 Session-Token 即可。**
            ...
            @st.experimental_fragment
            def account_data_area():
                fields = ['用户组', '服务商', '订阅类型', '账户邮箱', 'AC_Token', 'RF_Token']
                rows = [[groups, datas['service_provider'], datas['account_type'], datas['account'], datas['access_token'], datas['refresh_token']] for groups, datas in accounts.items()]
                account_df = pd.DataFrame(rows, columns=fields)
                ...
                with st.expander("**表格工具**", expanded=False, icon=":material/construction:"):
                    ...
                    if order_row == "升序":
                        account_df = account_df.sort_values(by=order_what, ascending=True)
                    elif order_row == "降序":
                        account_df = account_df.sort_values(by=order_what, ascending=False)

                edited_df1 = st.data_editor(account_df, hide_index=hide_index, use_container_width=True, num_rows="dynamic", height=height, column_config={"服务商": st.column_config.SelectboxColumn(options=["OpenAI", "Anthropic"]), "订阅类型": st.column_config.SelectboxColumn(options=["Free", "Plus"])}, column_order=order)
                ...
                with col6:
                    if st.button("**保存更改**", use_container_width=True, type=button_type):
                        ...
                        json_data1 = df_to_json_account(edited_df1)
                ...
                with col7:
                    if st.button("**新增账户**", use_container_width=True, type=button_type):
                        st.session_state.add_account = True
                        time.sleep(0.05)
                        st.rerun()
                with col8:
            ...
            st.info("""**开启后，用户登陆 / 共享站登录时会自动检测用户的 SA_Token 是否有效，当失败时会自动尝试刷新！**""", icon=':material/double_arrow:')
            st.write("")

            @st.experimental_fragment
            def account_auto_refresh_area():
                refresh = st.checkbox("**自动检测 SA/AC 状态**", value=web_setting["web"]["refresh_all"], help="**仅支持 Oaifree ，且您必须配置有效的 RF_Token 才能正常使用！** \n\n **此服务已对时间有限 `expires_in不为0` 的用户进行单独优化，开启后不会影响并重置此类用户的使用剩余时间！**")
                st.write("")
                col1, col2 = st.columns([0.2, 0.8])
                with col1:
                    if st.button("**保存更改**", use_container_width=True, key="save_AC_Token刷新", type=button_type):
                        ...
                        st.session_state.success = True
                        st.rerun()
            account_auto_refresh_area()


            st.divider()
            st.write("##### 手动刷新 AC_Token (Oaifree)")
            st.info("""
            **若无 RF_Token 可使用此服务手动刷新 AC_Token。**
            
            **注意：手动刷新并不会刷新用户的 SA_Token ，可前往 用户总览 - 状态检查 进行用户状态刷新！**
            """, icon=':material/double_arrow:')
            st.write("")
            ...
            @st.experimental_fragment
            def account_refresh_area():
                col1, col2 = st.columns([0.2, 0.8])
                col6, col7 = st.columns([0.99999, 0.00001])
                with col1:
                    if st.button(f"**刷新用户组**", use_container_width=True, type=button_type):
            ...
            def invite_data_area():
                ...
                with col2:
                    if st.button("**生成令牌**", use_container_width=True, key="new_invite_token", type=button_type):
                        st.session_state.invite_click = True
                        time.sleep(0.05)
                        st.rerun()
            invite_data_area()
    ...
    if set_choose == '刷新令牌':
        with st.container(border=True):
            ...
            def refresh_refresh_area():
                ...
                with col2:
                    if st.button("**生成令牌**", use_container_width=True, key="new_refresh_token", type=button_type):
                        st.session_state.refresh_click = True
                        time.sleep(0.05)
                        st.rerun()
            refresh_refresh_area()
            ...
            @st.experimental_fragment
            def refresh_refresh_claude_area():
                st.success("""**敬请期待...**""")
            refresh_refresh_claude_area()

            st.divider()
            st.write("##### 自助服务")

            @st.experimental_fragment
            def refresh_link_area():
    ...
    if set_choose == '用户总览':
        with st.container(border=True):
            ...
            **UID是用户的唯一标识，可用于修改密码、账户登录；ChatGPT、Claude 表示该用户是否可用此服务~**
            """, icon=':material/double_arrow:')
            st.write("")
                    ...
                    if st.button("**保存更改**", use_container_width=True, key="save_user_data", type=button_type):
                        ...
                        if not error:
                            ...
                            with open(current_path + '/users.json', 'w', encoding='utf-8') as json_file:
                                json_file.write(json_data)
                            with open(current_path + '/share.json', 'w', encoding='utf-8') as json_file:
                                json.dump(share_data, json_file, ensure_ascii=False, indent=2)
                            time.sleep(0.1)
                            st.session_state.success = True
                            st.rerun()
                        else:
                            with col3:
                                st.error("**警告：出于安全性考虑，不允许在表格内直接新增用户，请用下方注册功能重试！**", icon=":material/error:")
                ...
                with col1:
                    if st.button("**注册用户**", use_container_width=True, key="register_user_admin", type=button_type):
                        ...
                        if "signup1" in st.session_state:
                            ...
                            user_datas = json.dumps(user_data, indent=2)
                            with open(current_path + 'users.json', 'w', encoding='utf-8') as json_file:
    ...
    if set_choose == 'Oaifree':
        with st.container(border=True):
            ...
            st.info("""
            **此部分可查看 Oaifree 用户的详细数据，仅允许更改SA_Token、删除 Oaifree 用户，不允许新增用户！**
            
            **注意：此页面的删除为取消其 Oaifree 服务的使用权，并非严格意义上的删除！**
            """, icon=':material/double_arrow:')
            st.write("")

            @st.experimental_fragment
            def oaifree_data_area():
                ...
                with col1:
                    if st.button("**保存更改** ", use_container_width=True, key="save_user", type=button_type):
                        ...
                        if not error:
                            ...
                            with col6:
                                st.toast("**非法操作，请重试！**", icon=":material/error:")
                                st.error("**不允许在此表格新增用户！请使用 用户总览 - 用户注册 功能来新增 Oaifree 用户！**", icon=":material/error:")
                with col2:
                    if st.button("**状态刷新**", use_container_width=True, type=button_type):
                        st.session_state.refresh_user_click = True
                        time.sleep(0.05)
                        st.rerun()
                with col3:
    ...
    if set_choose == 'Fuclaude':
        with st.container(border=True):
            ...
            st.info("""
            **此部分可查看 Fuclaude 用户的详细数据，仅允许更改SE_Token、删除 Fuclaude 用户，不允许新增用户！**
            
            **注意：此页面的删除为取消其 Fuclaude 服务的使用权，并非严格意义上的删除！**
            """, icon=':material/double_arrow:')
            st.write("")
            @st.experimental_fragment
            def fuclaude_data_area():
    ...
    if set_choose == '共享服务':
        with st.container(border=True):
            ...
            if share:
                ...
                index = []
                for i in share_list:
                    index.append(list(user_data.keys()).index(i))
                share_account = sac.transfer(items=user_data.keys(), index=index, titles=['用户池', '共享池'],
    ...
    if set_choose == 'Linux.Do':
        with st.container(border=True):
            ...
            def linux_do_area():
                ...
                with col1:
                    CLIENT_ID = st.text_input("**Client ID**", value=secret_data["linux_do"]["CLIENT_ID"])
                with col2:
                    CLIENT_SECRET = st.text_input("**Client Secret**", value=secret_data["linux_do"]["CLIENT_SECRET"], type="password")
                with col3:
                    if st.button("**保存更改**", use_container_width=True, type=button_type):
                        secret_data["linux_do"]["OPENED"] = OPENED
                        secret_data["linux_do"]["CLIENT_ID"] = CLIENT_ID
                        secret_data["linux_do"]["CLIENT_SECRET"] = CLIENT_SECRET
                        with open(current_path + "secret.toml", "w", encoding="utf-8") as f:
            ...
            @st.experimental_fragment
            def linux_do_setting_area():
                col1, col2 = st.columns([0.2, 0.8])
                with col1:
                    gen = st.radio("What", ["**:green[信用级别 - 1]**", "**:blue[信用级别 - 2]**", "**:red[信用级别 - 3]**", "**:rainbow[信用级别 - 4]**"], captions=["基本用户", "成员", "活跃用户", "领导者"], label_visibility="collapsed")
                with col2:
                    if gen == "**:green[信用级别 - 1]**":
                        with st.form(key="linux_do_1"):
                            ...
                            with st.expander("**更多设置**", expanded=False):
                                col3, col4 = st.columns(2)
                                with col3:
                                    site_limit = st.text_input("**限制网站（为空不限制）**", help="限制的网站, 为空则不限制", placeholder="site1,site2", key=f"site{gen}", value=secret_data["linux_do_1"]["site_limit"])
                                    expires_in = st.number_input("**过期秒数（秒）**", help="token有效期（单位为秒），填 0 则永久有效", key=f"expires{gen}", value=secret_data["linux_do_1"]["expires_in"])
                                with col4:
                                    gpt35_limit = st.number_input("**限制GPT-3.5**", help="GPT-3.5对话限制，填 -1 则不限制", key=f"gpt35{gen}", value=secret_data["linux_do_1"]["gpt35_limit"])
                                    gpt4_limit = st.number_input("**限制GPT-4**", help="GPT-4对话限制，填 -1 则不限制", key=f"gpt4{gen}", value=secret_data["linux_do_1"]["gpt4_limit"])
                                show_conversations = st.selectbox("**会话无需隔离**", ['true', 'false'], help="false为隔离会话", key=f"show{gen}", index=['true', 'false'].index(secret_data["linux_do_1"]["show_conversations"]))
                            col3, col4, col5 = st.columns(3)
                            with col3:
                                if st.form_submit_button("**保存更改**", type="primary"):
                                    secret_data["linux_do_1"]["chatgpt"] = allow_chatgpt
                                    secret_data["linux_do_1"]["chatgpt_group"] = chatgpt_group
                                    secret_data["linux_do_1"]["claude"] = allow_claude
                                    secret_data["linux_do_1"]["claude_group"] = claude_group
                                    secret_data["linux_do_1"]["site_limit"] = site_limit
                                    secret_data["linux_do_1"]["expires_in"] = expires_in
                                    secret_data["linux_do_1"]["gpt35_limit"] = gpt35_limit
                                    secret_data["linux_do_1"]["gpt4_limit"] = gpt4_limit
                                    secret_data["linux_do_1"]["show_conversations"] = show_conversations
                                    with open(current_path + "secret.toml", "w", encoding="utf-8") as f:
                                        toml.dump(secret_data, f)
                                    time.sleep(0.05)
                                    st.session_state.success = True
                    ...
                    if gen == "**:blue[信用级别 - 2]**":
                        with st.form(key="linux_do_2"):
                            ...
                            with st.expander("**更多设置**", expanded=False):
                                col3, col4 = st.columns(2)
                                with col3:
                                    site_limit = st.text_input("**限制网站（为空不限制）**", help="限制的网站, 为空则不限制", placeholder="site1,site2", key=f"site{gen}", value=secret_data["linux_do_2"]["site_limit"])
                                    expires_in = st.number_input("**过期秒数（秒）**", help="token有效期（单位为秒），填 0 则永久有效", key=f"expires{gen}", value=secret_data["linux_do_2"]["expires_in"])
                                with col4:
                                    gpt35_limit = st.number_input("**限制GPT-3.5**", help="GPT-3.5对话限制，填 -1 则不限制", key=f"gpt35{gen}", value=secret_data["linux_do_2"]["gpt35_limit"])
                                    gpt4_limit = st.number_input("**限制GPT-4**", help="GPT-4对话限制，填 -1 则不限制", key=f"gpt4{gen}", value=secret_data["linux_do_2"]["gpt4_limit"])
                                show_conversations = st.selectbox("**会话无需隔离**", ['true', 'false'], help="false为隔离会话", key=f"show{gen}", index=['true', 'false'].index(secret_data["linux_do_2"]["show_conversations"]))
                            col3, col4, col5 = st.columns(3)
                            with col3:
                                if st.form_submit_button("**保存更改**", type="primary"):
                                    secret_data["linux_do_2"]["chatgpt"] = allow_chatgpt
                                    secret_data["linux_do_2"]["chatgpt_group"] = chatgpt_group
                                    secret_data["linux_do_2"]["claude"] = allow_claude
                                    secret_data["linux_do_2"]["claude_group"] = claude_group
                                    secret_data["linux_do_2"]["site_limit"] = site_limit
                                    secret_data["linux_do_2"]["expires_in"] = expires_in
                                    secret_data["linux_do_2"]["gpt35_limit"] = gpt35_limit
                                    secret_data["linux_do_2"]["gpt4_limit"] = gpt4_limit
                                    secret_data["linux_do_2"]["show_conversations"] = show_conversations
                                    with open(current_path + "secret.toml", "w", encoding="utf-8") as f:
                                        toml.dump(secret_data, f)
                                    time.sleep(0.05)
                                    st.session_state.success = True
                    ...
                    if gen == "**:red[信用级别 - 3]**":
                        with st.form(key="linux_do_3"):
                            ...
                            with st.expander("**更多设置**", expanded=False):
                                col3, col4 = st.columns(2)
                                with col3:
                                    site_limit = st.text_input("**限制网站（为空不限制）**", help="限制的网站, 为空则不限制", placeholder="site1,site2", key=f"site{gen}", value=secret_data["linux_do_3"]["site_limit"])
                                    expires_in = st.number_input("**过期秒数（秒）**", help="token有效期（单位为秒），填 0 则永久有效", key=f"expires{gen}", value=secret_data["linux_do_3"]["expires_in"])
                                with col4:
                                    gpt35_limit = st.number_input("**限制GPT-3.5**", help="GPT-3.5对话限制，填 -1 则不限制", key=f"gpt35{gen}", value=secret_data["linux_do_3"]["gpt35_limit"])
                                    gpt4_limit = st.number_input("**限制GPT-4**", help="GPT-4对话限制，填 -1 则不限制", key=f"gpt4{gen}", value=secret_data["linux_do_3"]["gpt4_limit"])
                                show_conversations = st.selectbox("**会话无需隔离**", ['true', 'false'], help="false为隔离会话", key=f"show{gen}", index=['true', 'false'].index(secret_data["linux_do_3"]["show_conversations"]))
                            col3, col4, col5 = st.columns(3)
                            with col3:
                                if st.form_submit_button("**保存更改**", type="primary"):
                                    secret_data["linux_do_3"]["chatgpt"] = allow_chatgpt
                                    secret_data["linux_do_3"]["chatgpt_group"] = chatgpt_group
                                    secret_data["linux_do_3"]["claude"] = allow_claude
                                    secret_data["linux_do_3"]["claude_group"] = claude_group
                                    secret_data["linux_do_3"]["site_limit"] = site_limit
                                    secret_data["linux_do_3"]["expires_in"] = expires_in
                                    secret_data["linux_do_3"]["gpt35_limit"] = gpt35_limit
                                    secret_data["linux_do_3"]["gpt4_limit"] = gpt4_limit
                                    secret_data["linux_do_3"]["show_conversations"] = show_conversations
                                    with open(current_path + "secret.toml", "w", encoding="utf-8") as f:
                                        toml.dump(secret_data, f)
                                    time.sleep(0.05)
                                    st.session_state.success = True
                                    st.rerun()
                    if gen == "**:rainbow[信用级别 - 4]**":
                        with st.form(key="linux_do_4"):
                            st.info("**不是哥们？你还真指望始皇能来？**", icon=':material/double_arrow:')
                            with st.expander("**我信！我就要设置捏~**", expanded=False):
                                col3, col4 = st.columns(2)
                                with col3:
                                ...
                                col3, col4 = st.columns(2)
                                with col3:
                                    site_limit = st.text_input("**限制网站（为空不限制）**", help="限制的网站, 为空则不限制", placeholder="site1,site2", key=f"site{gen}", value=secret_data["linux_do_4"]["site_limit"])
                                    expires_in = st.number_input("**过期秒数（秒）**", help="token有效期（单位为秒），填 0 则永久有效", key=f"expires{gen}", value=secret_data["linux_do_4"]["expires_in"])
                                with col4:
                                    gpt35_limit = st.number_input("**限制GPT-3.5**", help="GPT-3.5对话限制，填 -1 则不限制", key=f"gpt35{gen}", value=secret_data["linux_do_4"]["gpt35_limit"])
                                    gpt4_limit = st.number_input("**限制GPT-4**", help="GPt-4对话限制，填 -1 则不限制", key=f"gpt4{gen}", value=secret_data["linux_do_4"]["gpt4_limit"])
                                show_conversations = st.selectbox("**会话无需隔离**", ['true', 'false'], help="false为隔离会话", key=f"show{gen}", index=['true', 'false'].index(secret_data["linux_do_4"]["show_conversations"]))
                            col3, col4, col5 = st.columns(3)
                            with col3:
                                if st.form_submit_button("**保存更改**", type="primary"):
                                    secret_data["linux_do_4"]["chatgpt"] = allow_chatgpt
                                    secret_data["linux_do_4"]["chatgpt_group"] = chatgpt_group
                                    secret_data["linux_do_4"]["claude"] = allow_claude
                                    secret_data["linux_do_4"]["claude_group"] = claude_group
                                    secret_data["linux_do_4"]["site_limit"] = site_limit
                                    secret_data["linux_do_4"]["expires_in"] = expires_in
                                    secret_data["linux_do_4"]["gpt35_limit"] = gpt35_limit
                                    secret_data["linux_do_4"]["gpt4_limit"] = gpt4_limit
                                    secret_data["linux_do_4"]["show_conversations"] = show_conversations
                                    with open(current_path + "secret.toml", "w", encoding="utf-8") as f:
                                        toml.dump(secret_data, f)
                                    time.sleep(0.05)
                                    st.session_state.success = True
                                    st.rerun()
            linux_do_setting_area()
            st.write("")
    if set_choose == '关于项目':
        with st.container(border=True):
            st.write("")
            ...
            with col3:
                st.link_button("**Neo 博客**", "https://zhile.io/", use_container_width=True, type=button_type)
            with col4:
    ...
    if st.session_state.theme == "Simple White":
        ...
        with open(style_path + "//Retro_Orange.html", "r", encoding="utf-8") as file:
            Retro_Orange_html = file.read()
            Retro_Orange_html = Retro_Orange_html.replace("{{text}}", "Collect your thoughts")
            st.markdown(Retro_Orange_html, unsafe_allow_html=True)
